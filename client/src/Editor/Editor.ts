...
class Editor {
    ...
    constructor(document_key: string, hasWriteAccess: boolean) {
        this.hasWriteAccess = hasWriteAccess;
        this.init(document_key);
    }

    private async init(document_key: string) {
        ...
        this.sizes = {
            ...
            rows: Math.floor(config.canvasHeight / (Math.max(charHeight, config.lineHeight) + (config.fontPadding * 2))),
        ...
        this.service = new DocumentService(this.canvasContainer, this.editor, this.sizes, document_key, this.hasWriteAccess);
