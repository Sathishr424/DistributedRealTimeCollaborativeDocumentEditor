...
class DocumentRepository {
    async createDocument(document_key: string, user_id: number): Promise<MyDocument> {
        try {
            const [rows] = await pool.execute<MyDocument[]>("INSERT INTO documents (`document_key`, `owner`) VALUES (?,?)", [document_key, user_id]);
            return rows[0];
        } catch (err) {
    ...
    async getDocumentUserAccess(document_key: string, user_id: number): Promise<DocumentAccess> {
        try {
            const [rows] = await pool.execute<DocumentAccess[]>("SELECT * FROM documentaccess WHERE document_id IN (SELECT * FROM documents WHERE document_key=?) AND user_id=?", [document_key, user_id]);
            if (rows.length > 0) {
                return rows[0];
            }
            return {
